...
class IngestionService:
    ...
    async def ensure_collection(self, collection_name: str, chunks: list[IngestDocument]) -> None:
        ...
        try:
            ...
            vectors_config = {
                dense_name: rest.VectorParams(size=len(vector), distance=rest.Distance.COSINE)
            }
            await client.create_collection(
                collection_name=collection_name,
                vectors_config=vectors_config,
                sparse_vectors_config={
                    settings.QDRANT_SPARSE_VECTOR_NAME: rest.SparseVectorParams(
                        modifier=rest.Modifier.IDF,
                    )
                },
            )
        else:
            # Existing collections are intentionally left untouched. Legacy
            # unnamed dense collections continue using dense + payload BM25.
            pass
    ...
    async def upsert_chunks(
        ...
        supports_hybrid = (
            ...
            and dense_name in vectors_config
        ...
        for chunk_index, chunk in enumerate(chunks):
            ...
            if supports_hybrid:
                point_vector = {
                    dense_name: vector,
                    settings.QDRANT_SPARSE_VECTOR_NAME: retrieval_service._sparse_vector(chunk.page_content),
                }
            else:
