...
class LLMService:
    ...
    async def _chat(self, model: str, messages: list, json_mode: bool = False, timeout: float | None = None) -> str:
        ...
        resp = await client.post(
            ...
            json=payload,
            timeout=timeout or settings.LLM_ANSWER_TIMEOUT,
        )
    ...
    async def generate_smalltalk(self, question: str, runtime_context: dict):
        if not self._use_local:
        ...
        try:
            return await self._chat(self._model_light, messages, timeout=settings.LLM_ROUTER_TIMEOUT)
        except httpx.HTTPError:
            return "Xin chào! Mình có thể hỗ trợ bạn hỏi đáp dựa trên tài liệu, lịch sử hội thoại và knowledge base."

    async def generate_direct_answer(self, question: str, runtime_context: dict):
        if not self._use_local:
    ...
    async def generate_answer(self, question: str, rewritten_question: str, runtime_context: dict, selected_context: str, citations: list):
        ...
        user_content = (
            f"Câu hỏi: {question}\n\n"
            f"Context (chỉ dùng thông tin dưới đây):\n{selected_context}\n\n"
            "Trả lời ngắn gọn, chính xác bằng tiếng Việt, dựa hoàn toàn vào context trên."
        )
        messages = [
        ...
        try:
            return await self._chat(self._model_heavy, messages, timeout=settings.LLM_ANSWER_TIMEOUT)
        except httpx.HTTPError as exc:
