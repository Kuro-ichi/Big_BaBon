...
async def answer_generation_node(state):
    try:
        if state.get("safety_action") == "respond":
            ...
            # Sensitive answers must pass the output guard before any bytes are
            # exposed to a remote client. Normal answers keep true streaming.
            guarded_stream = state.get("risk_level") not in {"sensitive", "blocked"}
            state["answer"] = await llm_service.generate_answer(
                ...
                token_sink=state.get("token_sink") if guarded_stream else None,
            )
